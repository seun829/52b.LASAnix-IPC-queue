...
int lq_getattr (lqd_t __msgid, struct lq_attr *__mqstat)
{
	...
	if (lqueue_table[__msgid].lq_name[0] != 0)
	    ...
	    PQueue* q = lqueue_table[__msgid].lq_ptr;
		__mqstat->lq_flags = q->getFlag();
		__mqstat->lq_maxmsg = MAX_MESSAGES;
		__mqstat->lq_msgsize = MAX_MSG_SIZE;
		__mqstat->lq_curmsgs = q->getCur();
